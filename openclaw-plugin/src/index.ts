...
const EPHEMERAL_ROLLUP_VALIDATOR = new PublicKey(
...
const isComponentVisibleOnEr = async (runtime: Runtime, component: PublicKey) =>
  Boolean(await runtime.erConnection.getAccountInfo(component));

const isComponentDelegated = async (runtime: Runtime, component: PublicKey) =>
  Boolean(
    await runtime.baseConnection.getAccountInfo(findDelegationRecordPda(component))
  );

const waitForComponentOnEr = async (
  runtime: Runtime,
  component: PublicKey,
  label: string
) => {
  ...
  throw new Error(
    `${label} delegation did not appear on ER at ${runtime.config.erRpcUrl}.`
  );
};

const ensureTileItemInitialized = async (
  runtime: Runtime,
  ...
  if (!baseComponentInfo && !erComponentInfo) {
    if (!createIfMissing) {
      throw new Error(
        `No active tile item component ${tileItem.component.toBase58()} exists for that tile.`
      );
    }
    const { InitializeComponent } = await import("@magicblock-labs/bolt-sdk");
    await sendBoltResult(
...
const ensureTileItemDelegated = async (
  runtime: Runtime,
  tileItem: Awaited<ReturnType<typeof getTileItemRefs>>
) => {
  ...
  const baseAccount = await runtime.baseConnection.getAccountInfo(
    tileItem.component
  );
  if (!baseAccount) {
    throw new Error(
      `Cannot delegate missing tile item component ${tileItem.component.toBase58()}.`
    );
  }
...
const sendBoltResult = async (
  runtime: Runtime,
  result: BoltResult,
  connection: Connection = runtime.erConnection
) => {
  const transaction =
    result.transaction ??
